...
function update() {
  if (!gameStarted) {
    ...
    ctx.fillStyle = "white";
    ctx.font = "30px Arial";
    ctx.fillText("Press SPACE or Start", 90, canvas.height / 2);
    return;
  }
  ...
  for (let i = pipes.length - 1; i >= 0; i--) {
    ...
    if (checkCollision(bird, pipes[i])) gameOver = true;
  ...
  if (gameOver) {
    ...
    ctx.fillText("Press R or Start", 130, canvas.height / 2 + 40);
  }
}
...
const overlay = document.getElementById("landingScreen");
